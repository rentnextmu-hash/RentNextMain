...
**About / Contact / Terms / Privacy pages: built, not yet deployed.**

- `/about` and `/contact` — **invented placeholder copy** (at the user's request, pending the client's own). About's figures (cars, models, pickup points, from-price) are live from the DB; the prose avoids unverifiable claims (no founding year, no customer counts). Contact lists phone / WhatsApp / email from `settings` (new `companyWebsite` in `getPublicSettings`) and every location's address + hours — **deliberately no contact form**: there's no enquiries table or email provider, and a form that silently drops messages is worse than none.
- `/terms` and `/privacy` — honest placeholders ("being finalised"), `noindex`. **No legal text was invented**; real terms and a privacy notice must come from the client. The booking flow's "I accept the rental terms" checkbox points at this.
- Public `not-found.tsx` now has an sr-only `<h1>`.

**Block B9 (part) — Locations admin (P9.1) + hotel partners (P9.2): built and verified locally; migration 0010 NOT yet applied live.**

- `/admin/locations`: cards in display order with a stacked available/booked/maintenance bar and pickups this month (`getLocationsAdmin`, `mauritiusMonthRange()` in lib/format.ts). **Reorder** by HTML5 drag or up/down buttons (keyboard) → `reorderLocationsAction` saves immediately; the public `/locations` order follows.
- `/admin/locations/[slug]`: stat cards, cars based here (with who has them now), next 7 days of pickups/returns there, partner hotels, and a "Public page" panel showing the live-priced SEO title/description/intro + "View live".
- `components/admin/locations/LocationForm.tsx` (Drawer, create + edit): every column incl. opening hours, drive-time rows and FAQ rows; slug auto-derived from the name for new locations, warns that changing it moves the public URL; SEO title / meta description counters warn past 60 / 160; `locationFormSchema`. `saveLocationAction` revalidates the admin pages, `/locations`, `/locations/[old+new slug]`, `/contact`, `/sitemap.xml` and `/` so edits reach the (statically generated) public site immediately.
- `/admin/hotels`: stat cards (active partners; referred bookings, revenue and commission this month), status/location filters (GET form), table. `/admin/hotels/[slug]`: contract, contact (mailto / tel / WhatsApp), performance this month + all time with **commission owed**, all linked bookings tagged "Referred" vs "Delivery only", disabled "Partner portal" preview. `HotelForm` drawer (create + edit), `hotelFormSchema`; saving revalidates the hotel's old and new location pages (their "Partner hotels nearby" list).
- **Commission rule**: only on bookings the hotel referred (`source = 'hotel'`), by pickup date, not cancelled — website customers who chose delivery to that hotel share its `hotel_id` but earn it nothing. Maths in `calculateCommission()` in lib/pricing.ts.
- **Migration 0010**: `hotels.contract_start_date` (nullable).
- All location/hotel writes need owner or manager (`requireRole`); staff see read-only pages with no edit/reorder controls.
- Verified locally (local stack, owner + staff): reorder by buttons and by drag with DB + public order following; location edit (bad slug rejected, 160-char warning, new FAQ and hours live on the public page); add location → public page + 7th map marker → hide → not found; hotel commission matches a direct SQL calculation (Rs 13,500 × 12% = Rs 1,620); bad email caught without losing input; new hotel appears in the public location page's partner list; tablet has no overflow; no browser errors.

**Block B9 (part) — Settings page (P9.3a): built and verified locally; migration 0011 NOT yet applied live.**

- `/admin/settings` (**owner only** — `getCurrentStaff().role === "owner"`, else an "Owner access only" panel; managers/staff can't change rates or company info): three sections.
- **Company + Booking** (`SettingsForms.tsx` → `saveSettingsAction`, owner-only via `requireRole(["owner"])`): upserts `settings` key/value rows. New keys (migration 0011 + seed): `company_email`, `company_whatsapp`, `company_address`, `advance_booking_days`, `cancellation_hours`. `getPublicSettings`/`PublicSettings` widened (companyTagline/Whatsapp/Email/Address, cancellationHours); `getAdminSettings` returns the editable set grouped company/booking.
- **Vehicle rates & details** (`CategorySettings.tsx`): inline edit of the three duration rates per row (per-row Save appears when changed, disabled if rates don't fall 1–2 ≥ 3–5 ≥ 6+), plus a full edit Drawer (name, tagline, best-for, description, specs, rates, active). `saveCategoryRatesAction` / `saveCategoryAction`. **Editing a rate flows straight to the public `/cars`, `/cars/[slug]` "from" price** (revalidated). Categories are edit-only (they mirror the fixed real fleet), not created here.
- **Extras** (`AddOnSettings.tsx`): list + add/edit Drawer (`saveAddOnAction`, create slugifies the name with a clash suffix). Deactivating removes it from the public booking extras; adding shows it there.
- `settingsFormSchema` / `categoryFormSchema` / `categoryRatesSchema` / `addOnFormSchema` in validation.ts. New shared `components/ui/useToast.ts`. Sidebar "Settings" enabled.
- **Gotcha fixed**: a `<Toast>` (`<div>`) rendered inside a `<tr>` (invalid HTML, hydration warning) — the inline-rate toast is now hoisted to the table via an `onSaved` callback.
- Verified locally (owner + staff): company/min-days save reaches DB + public footer/title; bad time (`25:99`) rejected; inline Vitz rate edit 3000→2800 shows on `/cars/toyota-vitz`; rate-order rejected inline and in the drawer; add-on "Wi-Fi hotspot" appears in the public booking extras; deactivating GPS hides it; staff hits the owner-only gate (no Save button); no tablet overflow; no hydration/console errors.

**Block B9 (part) — Polish pass (P9.3c): built and verified locally.**

- **Boundaries**: `app/(admin)/admin/{loading,error,not-found}.tsx` (skeleton via new `components/ui/Skeleton.tsx`; client error boundary with retry + digest; styled admin not-found for `notFound()` calls), `app/(public)/error.tsx`, and root `app/global-error.tsx` (self-contained html/body).
- **OG images** (next/og `ImageResponse`, no external fonts): `app/opengraph-image.tsx` (brand default for every page), `cars/[slug]/opengraph-image.tsx` (name + from-price + embedded car photo — `lib/og.tsx` `loadCarImageDataUri` reads the public asset from disk first, then CDN fallback), `locations/[slug]/opengraph-image.tsx` ("Car Rental in/at X" + from-price). Shared `ogFrame()` in `lib/og.tsx`. Removed the car page's explicit `openGraph.images` (a 1:1 cutout) so the dynamic card wins.
- **Admin titles**: `(admin)/admin/layout.tsx` sets a `title` template (`%s · Rent Next Admin`, `robots: noindex`); each admin page exports its own title (fixes every admin tab reading "Rent Next Car Hire" and an axe document-title finding).
- **Accessibility (axe-core wcag2a/2aa sweep, every key page ✓ clean)**: darkened `--color-text-muted` `#6b7280 → #5b6472` (was 4.29:1 on surface-alt, now ≥5.3); added `--color-{success,warning,error,info}-deep` tokens for badge/pill text on the 10% tints (Badge uses them) — success/warning were failing; swept body `text-warning → text-warning-deep` (3.34→AA on white); brighter disabled sidebar items (`white/40 → white/60`); car spec grid `dl/dt/dd → ul/li/p` (dt/dd weren't direct dl children); `MauritiusMap` dropped `role="img"` (it has link descendants — was nested-interactive) for `aria-label`; unavailable car-step cards use a muted background instead of `opacity` (kept text at full contrast). New `components/ui/useToast.ts` shared hook.
- Verified: axe serious/critical clean on home, catalogue, car, search, locations, location, about, contact, booking car step, and admin dashboard/bookings/settings/new-booking/locations/hotels/fleet; OG images render as valid 1200×630 PNGs; admin titles non-empty; `tsc`/`eslint`/`next build` clean.

**Still to do in P9.3**: image upload (P9.3b) — Supabase Storage buckets/policies + an `ImageUpload` component wired into the category and location edit forms; **can't be exercised against the local stack** (no `storage` schema / storage-api), so it needs a guarded migration and verification against the live project after deploy.

**Next up:** push migrations 0010 + 0011 live together, then deploy. Then finish P9.3: image upload (P9.3b) and the polish pass (P9.3c).
