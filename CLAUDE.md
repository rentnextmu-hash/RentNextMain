...
**Block B5 — Booking journey (P5.1–P5.4): built and verified locally; migration 0006 + Edge Functions NOT yet applied to the live project.**
...
- **Migration 0006**: `add_ons.max_quantity`, `customers.marketing_consent`, `bookings.terms_accepted_at`, `public_partner_hotels` view (anon-readable hotel names only, same pattern as 0005), `create_booking_request()` (execute revoked from anon/authenticated). `types/database.ts` hand-merged with the 0006 additions from a local-DB generation — **re-run `npm run types:gen` after pushing 0006 live.**
...
- **Not done yet**: live `supabase db push` of 0006, `supabase secrets set BOOKING_LINK_SECRET=…`, deploying the four functions, regenerating types from live, a live end-to-end booking. Emails also need `RESEND_API_KEY` + a verified sending domain + `SITE_URL`. Website bookings are `requested` with no vehicle and don't consume inventory until staff confirm+assign (V1 by design).

**Next up:** push/deploy B5 live (needs owner approval), then B8 admin bookings list/detail so staff can confirm and assign the requests the site now creates.
